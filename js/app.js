...
window.onload = function () {
	...
	function updateCart(productButton, productId, productAdd = true) {
		...
		const cartIcon = cart.querySelector('.cart-menu__cart-icon');
		...
		if (productAdd) {
			...
			if (!cartProduct) {
				...
				const cartPrice = product.querySelector('.card__price');
				const price = parseInt(cartPrice.innerHTML);
				console.log(price);
...
navItem.forEach(el => {
	...
	el.addEventListener('mouseleave', (e) => {
		//navLine.style.width = `${navItem[0].offsetWidth}px`;
		navLine.style.width = `${0}px`;
		navLine.style.left = `47%`;
		// navLine.style.left = `-20%`;
	});
});

//Сортировка
let list = document.querySelectorAll('.shop__list');
let itembox = document.querySelectorAll('.shop__card');
